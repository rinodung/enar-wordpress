<?php
	...
	$thumb 		  = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'enar-blog-masonry' );
	$thumb 		  = $thumb['0'];
	...
	if ( $portfolio_type == 'image' || $portfolio_type == '' ) {
		...
		if ( $video_iframe !== '' ) {
			$output .= '<div class="embed-container">'.$video_iframe.'</div>';
		...
		if ( $audio_iframe !== '' ) {
			$output .= '<div class="embed-container">'.$audio_iframe.'</div>';
		...
		if ( $portfolio_type == 'gallery' ){
			...
			if ($gallery !== ''){
				foreach($gallery as $gall) {							
					...
					$gall_thumb_img = wp_get_attachment_image_src($imgid, 'enar-blog-masonry');											
