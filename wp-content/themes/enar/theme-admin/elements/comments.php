<?php
class idealtheme_gregsThreadedCommentNumbering {
	...
	function comment_numbering( $comment_ID, $args = array(), $wrapclass = 'trees_number', $mode = 'echo' ) {
...
if ( ! function_exists( 'idealtheme_comment' ) ) :
function idealtheme_comment( $comment, $args, $depth ) {
	...
	?>
	<li <?php comment_class('single_comment comment-box'); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php esc_html_e( 'Pingback:', 'theme' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'theme' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	...
	<li <?php comment_class('single_comment'); ?> id="li-comment-<?php comment_ID(); ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment-container comment-box">
		<?php if (idealtheme_option('comment_numbering') == 1) { echo idealtheme_comment_numbers_comment_numbering($comment->comment_ID, $args); }  ?>
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'theme' ); ?></em>
			<?php endif; ?>
			<?php
			...
			<div class="comment_content">
			 <?php
				printf( '<h4 class="author_name fn">%1$s %2$s</h4>',
						idealtheme_fun_comment_author_url(get_comment_author_link()),
            ...
            ?>
			<?php
				printf( '<span class="comment_meta commentmetadata "><a href="%1$s"><time datetime="%2$s">%3$s</time></a></span>',
					esc_url( get_comment_link( $comment->comment_ID ) ),
					get_comment_time( 'c' ),
					/* translators: 1: date, 2: time */
					sprintf( esc_html__( '%1$s at %2$s', 'theme' ), get_comment_date(), get_comment_time() )
				);

            ?>

            <div class="comment_said_text"><?php comment_text(); ?></div>
            <?php comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'theme' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			<?php edit_comment_link( esc_html__( 'Edit', 'theme' )); ?>
			</div>
...
$enar_idealtheme_comment_numbers = new idealtheme_gregsThreadedCommentNumbering('idealtheme_comment_numbers', '1.5.3', "Greg's Threaded Comment Numbering");
function idealtheme_comment_numbers_comment_numbering($comment_ID, $args, $wrapclass = 'trees_number', $mode = 'echo') {
	global $enar_idealtheme_comment_numbers;
	return $enar_idealtheme_comment_numbers->comment_numbering($comment_ID, $args, $wrapclass, $mode);
}
