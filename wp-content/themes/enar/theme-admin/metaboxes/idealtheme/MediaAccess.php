						...
						$('[class*=<?php echo $this->button_class_name; ?>]').live('click', function(e)
						{
							...
							customMediaManager.on('select', function(){
								...
								if (media_attachment.type === 'image') {
									formlabel.find('.hamdan_mb_image_img_id').val(media_attachment.id);
									var thumb = '';
									...
									formlabel.find('input.hamdan_img_src_input').val(thumb);
									//formlabel.find('input.hamdan_full_meta_input').val(media_attachment.sizes['full']['url']);
									formlabel.find('.hamdan_gallery_img_src img').attr('src',thumb);
									formlabel.find('.hamdan_gallery_img_src').find('.mti_remove_img').show();
								} else {
