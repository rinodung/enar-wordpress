...
?>
<tr class="shipping">
	<th><h4 class="upper roboto_font"><?php
	...
	<td>
		...
		<?php elseif ( ( WC()->countries->get_states( WC()->customer->get_shipping_country() ) && ! WC()->customer->get_shipping_state() ) || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
				...
				<p class="description5"><?php esc_html_e( 'Please use the shipping calculator to see available shipping methods.', 'woocommerce' ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p class="description5"><?php esc_html_e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', 'woocommerce' ); ?></p>

			<?php else : ?>

				<p class="description5"><?php esc_html_e( 'Please fill in your details to see available shipping methods.', 'woocommerce' ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<p class="description5">' . esc_html__( 'There are no shipping methods available. Please double check your address, or contact us if you need any help.', 'woocommerce' ) . '</p>'
				); ?>
